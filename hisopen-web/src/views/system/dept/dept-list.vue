...
<script>
import deptApi from '@/api/system/dept/dept'
export default {
  data() {
    return {
      // 是否启用遮罩层
  ...
  methods: {
    ...
    getDeptList() {
      this.loading = true
      deptApi.listForPage(this.addDateRange(this.queryParams, this.dateRange)).then(res => {
    ...
    handleUpdate(row) {
      ...
      // 根据dictId查询一个字典信息
      this.loading = true
      deptApi.getDeptById(deptId).then(res => {
        this.form = res.data
        this.loading = false
      })
    },
    ...
    handleDelete(row) {
      ...
      this.$confirm('此操作将永久删除该科室数据, 是否继续?', '提示', {
        ...
        this.loading = true
        deptApi.deleteByIds(deptIds).then(res => {
          this.loading = false
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.deptId === undefined) {
            deptApi.addDept(this.form).then(res => {
            ...
            deptApi.updateDept(this.form).then(res => {
